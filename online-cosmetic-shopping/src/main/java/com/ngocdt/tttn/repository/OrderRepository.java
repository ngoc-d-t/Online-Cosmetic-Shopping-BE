...
public interface OrderRepository extends JpaRepository<Order, Integer> {
    ...
    @Query(value = "select top 1 f.employeeID from (select A.employeeID, isnull((select quantity from (select count(1) quantity, o.shipperID from Orders o where o.state in ('confirmed', 'DELIVERED','DELIVERING') and ISNULL(o.shipperID, 0) != 0 and CAST(o.date as nchar(7)) = CAST(CAST(getdate() as date) as nchar(7)) group by o.shipperID) s where s.shipperID = a.employeeID), 0) quantity from (select roleID from role where roleName='ROLE_SHIPPER') r INNER JOIN Account a on a.roleID = r.roleID) f order by f.quantity", nativeQuery = true)
    Integer findShipper();
